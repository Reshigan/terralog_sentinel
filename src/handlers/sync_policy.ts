...
const FIELDS: Spec[] = [{ name: "name", type: "text" }, { name: "max_attempts", type: "integer" }, { name: "retry_interval_seconds", type: "integer" }, { name: "description", type: "text" }, { name: "is_active", type: "bool" }, { name: "backoff_strategy", type: "enum", values: ["linear","exponential","fixed"] }, { name: "min_backoff_seconds", type: "integer" }, { name: "max_backoff_seconds", type: "integer" }, { name: "created_by", type: "text" }, { name: "created_at", type: "date" }];
...
const SORTABLE: string[] = ["id","name","max_attempts","retry_interval_seconds","description","is_active","backoff_strategy","min_backoff_seconds","max_backoff_seconds","created_by","created_at"];
const SEARCHABLE: string[] = ["name","description","backoff_strategy","created_by","created_at"];
const FILTERABLE: string[] = [];
const MATCHABLE: string[] = ["backoff_strategy"];
const DATED: string[] = ["created_at"];
const GROUPABLE: string[] = ["is_active","backoff_strategy","created_at"];
const MEASURABLE: string[] = ["max_attempts","retry_interval_seconds","min_backoff_seconds","max_backoff_seconds"];
...
export const listSync_policys: ListSync_policysHandler = async (req, env) => {
  ...
  const { where, args } = sc;
  const counted = await env.DB.prepare("SELECT COUNT(*) AS n, COALESCE(SUM(max_attempts), 0) AS s_max_attempts, COALESCE(SUM(retry_interval_seconds), 0) AS s_retry_interval_seconds, COALESCE(SUM(min_backoff_seconds), 0) AS s_min_backoff_seconds, COALESCE(SUM(max_backoff_seconds), 0) AS s_max_backoff_seconds FROM sync_policys WHERE " + where).bind(...args).first<Record<string, number>>();
  let pageWhere = where;
  const pageArgs = [...args];
  ...
  if (counted) out.headers.set("x-totals", JSON.stringify({ max_attempts: counted.s_max_attempts ?? 0, retry_interval_seconds: counted.s_retry_interval_seconds ?? 0, min_backoff_seconds: counted.s_min_backoff_seconds ?? 0, max_backoff_seconds: counted.s_max_backoff_seconds ?? 0 }));
  return out;
};
...
export const createSync_policy: CreateSync_policyHandler = async (req, env) => {
  ...
  try {
    const res = await env.DB.prepare("INSERT INTO sync_policys (name, max_attempts, retry_interval_seconds, description, is_active, backoff_strategy, min_backoff_seconds, max_backoff_seconds, created_by, created_at, tenant) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(body.name ?? null, body.max_attempts ?? null, body.retry_interval_seconds ?? null, body.description ?? null, body.is_active ?? null, body.backoff_strategy ?? null, body.min_backoff_seconds ?? null, body.max_backoff_seconds ?? null, body.created_by ?? null, body.created_at ?? null, user.tenant).run();
...
export const createSync_policyBulk: CreateSync_policyBulkHandler = async (req, env) => {
  ...
  try {
    const stmt = env.DB.prepare("INSERT INTO sync_policys (name, max_attempts, retry_interval_seconds, description, is_active, backoff_strategy, min_backoff_seconds, max_backoff_seconds, created_by, created_at, tenant) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING id");
    const out = await env.DB.batch<{ id: number }>(
      rows.map((body) => stmt.bind(body.name ?? null, body.max_attempts ?? null, body.retry_interval_seconds ?? null, body.description ?? null, body.is_active ?? null, body.backoff_strategy ?? null, body.min_backoff_seconds ?? null, body.max_backoff_seconds ?? null, body.created_by ?? null, body.created_at ?? null, user.tenant)),
    );
...
export const updateSync_policy: UpdateSync_policyHandler = async (req, env, params) => {
  ...
  let sql = "UPDATE sync_policys SET name = COALESCE(?, name), max_attempts = COALESCE(?, max_attempts), retry_interval_seconds = COALESCE(?, retry_interval_seconds), description = COALESCE(?, description), is_active = COALESCE(?, is_active), backoff_strategy = COALESCE(?, backoff_strategy), min_backoff_seconds = COALESCE(?, min_backoff_seconds), max_backoff_seconds = COALESCE(?, max_backoff_seconds), created_by = COALESCE(?, created_by), created_at = COALESCE(?, created_at), row_version = row_version + 1 WHERE id = ? AND tenant = ? AND deleted_at IS NULL";
  const args: unknown[] = [body.name ?? null, body.max_attempts ?? null, body.retry_interval_seconds ?? null, body.description ?? null, body.is_active ?? null, body.backoff_strategy ?? null, body.min_backoff_seconds ?? null, body.max_backoff_seconds ?? null, body.created_by ?? null, body.created_at ?? null, params.id, user.tenant];
