...
import type { Sync_log, Bucket, ListSync_logsHandler, ListSync_logsStatsHandler, GetSync_logHandler, CreateSync_logHandler, CreateSync_logBulkHandler, UpdateSync_logHandler } from "../types";
...
export const listSync_logs: ListSync_logsHandler = async (req, env) => {
  ...
  const { results } = await env.DB.prepare(
    "SELECT * FROM sync_logs WHERE " + pageWhere + " ORDER BY " + sort + " " + dir + " LIMIT ? OFFSET ?",
  ).bind(...pageArgs, per, cursor > 0 && sort === "id" ? 0 : (page - 1) * per).all<Sync_log>();
...
export const listSync_logsStats: ListSync_logsStatsHandler = async (req, env) => {
  const user = await requireSession(req, env);
  if (!user) return json({ error: "unauthenticated" }, 401);
  noteTenant(req, user.tenant);
  const url = new URL(req.url);
  ...
  const from = hop
    ? "sync_logs JOIN (SELECT id AS \"hop.id\", " + hopCol + " AS \"hop.key\" FROM " + hop.table + " WHERE tenant = ? AND deleted_at IS NULL) ON \"hop.id\" = " + by.slice(0, dot)
    : "sync_logs";
  const { results } = await env.DB.prepare(
...
export const getSync_log: GetSync_logHandler = async (req, env, params) => {
  const user = await requireSession(req, env);
  if (!user) return json({ error: "unauthenticated" }, 401);
  noteTenant(req, user.tenant);
  const row = await env.DB.prepare("SELECT * FROM sync_logs WHERE id = ? AND tenant = ? AND deleted_at IS NULL").bind(params.id, user.tenant).first<Sync_log>();
...
export const createSync_log: CreateSync_logHandler = async (req, env) => {
  const user = await requireSession(req, env);
  if (!user) return json({ error: "unauthenticated" }, 401);
  noteTenant(req, user.tenant);
  const denied = requireRole(user, WRITE_ROLES);
  if (denied) return denied;
  const body = await readJson<Partial<Sync_log>>(req);
  ...
  if (key) {
    const seen = await env.DB.prepare("SELECT row_id FROM idempotency WHERE tenant = ? AND key = ? AND endpoint = ?").bind(user.tenant, key, "createSync_log").first<{ row_id: number }>();
    if (seen) {
      const prior = await env.DB.prepare("SELECT * FROM sync_logs WHERE id = ? AND tenant = ? AND deleted_at IS NULL").bind(seen.row_id, user.tenant).first<Sync_log>();
      if (prior) {
  ...
  try {
    ...
    if (key)
      await env.DB.prepare("INSERT OR IGNORE INTO idempotency (tenant, key, endpoint, row_id, at) VALUES (?, ?, ?, ?, ?)")
        .bind(user.tenant, key, "createSync_log", Number(res.meta.last_row_id), new Date().toISOString()).run();
    await audit(env, user, "create", "sync_log", Number(res.meta.last_row_id), row);
...
export const createSync_logBulk: CreateSync_logBulkHandler = async (req, env) => {
  const user = await requireSession(req, env);
  if (!user) return json({ error: "unauthenticated" }, 401);
  noteTenant(req, user.tenant);
  const denied = requireRole(user, WRITE_ROLES);
  if (denied) return denied;
  const payload = await readJson<{ rows?: Partial<Sync_log>[] }>(req);
  ...
  for (let i = 0; i < rows.length; i++) {
    const bad = validate(rows[i] as Partial<Sync_log>);
...
export const updateSync_log: UpdateSync_logHandler = async (req, env, params) => {
  const user = await requireSession(req, env);
  if (!user) return json({ error: "unauthenticated" }, 401);
  noteTenant(req, user.tenant);
  const denied = requireRole(user, WRITE_ROLES);
  ...
  try {
    ...
    if (!res.meta.changes) {
      const cur = await env.DB.prepare("SELECT row_version FROM sync_logs WHERE id = ? AND tenant = ? AND deleted_at IS NULL").bind(params.id, user.tenant).first<{ row_version: number }>();
    ...
    const row = await env.DB.prepare("SELECT * FROM sync_logs WHERE id = ? AND tenant = ? AND deleted_at IS NULL").bind(params.id, user.tenant).first<Sync_log>();
    if (row) await audit(env, user, "update", "sync_log", Number(params.id), body);
