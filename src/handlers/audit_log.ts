...
import type { Audit_log, Bucket, ListAudit_logsHandler, ListAudit_logsStatsHandler, GetAudit_logHandler, CreateAudit_logHandler, CreateAudit_logBulkHandler, UpdateAudit_logHandler } from "../types";
...
const FIELDS: Spec[] = [{ name: "entity_type", type: "text" }, { name: "entity_id", type: "integer" }, { name: "action", type: "enum", values: ["create","update","delete","sync","erase"] }, { name: "performed_by", type: "text" }, { name: "performed_at", type: "date" }, { name: "metadata", type: "text" }, { name: "ip_address", type: "text" }, { name: "user_agent", type: "text" }, { name: "changes", type: "text" }, { name: "session_id", type: "text" }];
function validate(body: Partial<Audit_log>, partial = false): string | null {
...
const SORTABLE: string[] = ["id","entity_type","entity_id","action","performed_by","performed_at","metadata","ip_address","user_agent","changes","session_id"];
const SEARCHABLE: string[] = ["entity_type","action","performed_by","performed_at","metadata","ip_address","user_agent","changes","session_id"];
const FILTERABLE: string[] = [];
const MATCHABLE: string[] = ["action"];
const DATED: string[] = ["performed_at"];
const GROUPABLE: string[] = ["action","performed_at"];
const MEASURABLE: string[] = ["entity_id"];
...
export const listAudit_logs: ListAudit_logsHandler = async (req, env) => {
  ...
  const { where, args } = sc;
  const counted = await env.DB.prepare("SELECT COUNT(*) AS n, COALESCE(SUM(entity_id), 0) AS s_entity_id FROM audit_logs WHERE " + where).bind(...args).first<Record<string, number>>();
  let pageWhere = where;
  const pageArgs = [...args];
  ...
  const { results } = await env.DB.prepare(
    "SELECT * FROM audit_logs WHERE " + pageWhere + " ORDER BY " + sort + " " + dir + " LIMIT ? OFFSET ?",
  ).bind(...pageArgs, per, cursor > 0 && sort === "id" ? 0 : (page - 1) * per).all<Audit_log>();
  ...
  if (counted) out.headers.set("x-totals", JSON.stringify({ entity_id: counted.s_entity_id ?? 0 }));
  return out;
};

export const listAudit_logsStats: ListAudit_logsStatsHandler = async (req, env) => {
  const user = await requireSession(req, env);
  if (!user) return json({ error: "unauthenticated" }, 401);
  noteTenant(req, user.tenant);
  const url = new URL(req.url);
  ...
  const from = hop
    ? "audit_logs JOIN (SELECT id AS \"hop.id\", " + hopCol + " AS \"hop.key\" FROM " + hop.table + " WHERE tenant = ? AND deleted_at IS NULL) ON \"hop.id\" = " + by.slice(0, dot)
    : "audit_logs";
  const { results } = await env.DB.prepare(
...
export const getAudit_log: GetAudit_logHandler = async (req, env, params) => {
  const user = await requireSession(req, env);
  if (!user) return json({ error: "unauthenticated" }, 401);
  noteTenant(req, user.tenant);
  const row = await env.DB.prepare("SELECT * FROM audit_logs WHERE id = ? AND tenant = ? AND deleted_at IS NULL").bind(params.id, user.tenant).first<Audit_log>();
...
export const createAudit_log: CreateAudit_logHandler = async (req, env) => {
  const user = await requireSession(req, env);
  if (!user) return json({ error: "unauthenticated" }, 401);
  noteTenant(req, user.tenant);
  const denied = requireRole(user, WRITE_ROLES);
  if (denied) return denied;
  const body = await readJson<Partial<Audit_log>>(req);
  ...
  if (key) {
    const seen = await env.DB.prepare("SELECT row_id FROM idempotency WHERE tenant = ? AND key = ? AND endpoint = ?").bind(user.tenant, key, "createAudit_log").first<{ row_id: number }>();
    if (seen) {
      const prior = await env.DB.prepare("SELECT * FROM audit_logs WHERE id = ? AND tenant = ? AND deleted_at IS NULL").bind(seen.row_id, user.tenant).first<Audit_log>();
      if (prior) {
  ...
  try {
    const res = await env.DB.prepare("INSERT INTO audit_logs (entity_type, entity_id, action, performed_by, performed_at, metadata, ip_address, user_agent, changes, session_id, tenant) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(body.entity_type ?? null, body.entity_id ?? null, body.action ?? null, body.performed_by ?? null, body.performed_at ?? null, body.metadata ?? null, body.ip_address ?? null, body.user_agent ?? null, body.changes ?? null, body.session_id ?? null, user.tenant).run();
    const row = await env.DB.prepare("SELECT * FROM audit_logs WHERE id = ?").bind(res.meta.last_row_id).first<Audit_log>();
    if (key)
      await env.DB.prepare("INSERT OR IGNORE INTO idempotency (tenant, key, endpoint, row_id, at) VALUES (?, ?, ?, ?, ?)")
        .bind(user.tenant, key, "createAudit_log", Number(res.meta.last_row_id), new Date().toISOString()).run();
    await audit(env, user, "create", "audit_log", Number(res.meta.last_row_id), row);
...
export const createAudit_logBulk: CreateAudit_logBulkHandler = async (req, env) => {
  const user = await requireSession(req, env);
  if (!user) return json({ error: "unauthenticated" }, 401);
  noteTenant(req, user.tenant);
  const denied = requireRole(user, WRITE_ROLES);
  if (denied) return denied;
  const payload = await readJson<{ rows?: Partial<Audit_log>[] }>(req);
  ...
  for (let i = 0; i < rows.length; i++) {
    const bad = validate(rows[i] as Partial<Audit_log>);
    if (bad) return json({ error: "row " + i + ": " + bad }, 400);
  }
  try {
    const stmt = env.DB.prepare("INSERT INTO audit_logs (entity_type, entity_id, action, performed_by, performed_at, metadata, ip_address, user_agent, changes, session_id, tenant) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING id");
    const out = await env.DB.batch<{ id: number }>(
      rows.map((body) => stmt.bind(body.entity_type ?? null, body.entity_id ?? null, body.action ?? null, body.performed_by ?? null, body.performed_at ?? null, body.metadata ?? null, body.ip_address ?? null, body.user_agent ?? null, body.changes ?? null, body.session_id ?? null, user.tenant)),
    );
    const ids = out.flatMap((r) => (r.results ?? []).map((x) => x.id));
    for (const id of ids) await audit(env, user, "create", "audit_log", id, null);
...
export const updateAudit_log: UpdateAudit_logHandler = async (req, env, params) => {
  const user = await requireSession(req, env);
  if (!user) return json({ error: "unauthenticated" }, 401);
  noteTenant(req, user.tenant);
  const denied = requireRole(user, WRITE_ROLES);
  ...
  let sql = "UPDATE audit_logs SET entity_type = COALESCE(?, entity_type), entity_id = COALESCE(?, entity_id), action = COALESCE(?, action), performed_by = COALESCE(?, performed_by), performed_at = COALESCE(?, performed_at), metadata = COALESCE(?, metadata), ip_address = COALESCE(?, ip_address), user_agent = COALESCE(?, user_agent), changes = COALESCE(?, changes), session_id = COALESCE(?, session_id), row_version = row_version + 1 WHERE id = ? AND tenant = ? AND deleted_at IS NULL";
  const args: unknown[] = [body.entity_type ?? null, body.entity_id ?? null, body.action ?? null, body.performed_by ?? null, body.performed_at ?? null, body.metadata ?? null, body.ip_address ?? null, body.user_agent ?? null, body.changes ?? null, body.session_id ?? null, params.id, user.tenant];
  if (Number.isInteger(want)) { sql += " AND row_version = ?"; args.push(want); }
  try {
    const res = await env.DB.prepare(sql).bind(...args).run();
    if (!res.meta.changes) {
      const cur = await env.DB.prepare("SELECT row_version FROM audit_logs WHERE id = ? AND tenant = ? AND deleted_at IS NULL").bind(params.id, user.tenant).first<{ row_version: number }>();
    ...
    const row = await env.DB.prepare("SELECT * FROM audit_logs WHERE id = ? AND tenant = ? AND deleted_at IS NULL").bind(params.id, user.tenant).first<Audit_log>();
    if (row) await audit(env, user, "update", "audit_log", Number(params.id), body);
