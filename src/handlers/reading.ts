...
import type { Reading, Row, Bucket, ListOutliersHandler, SyncStatusHeatmapHandler, SiteReadingTrendHandler, PendingSyncReadingsHandler, FailedSyncThresholdReadingsHandler, DedupeCheckHandler, ListReadingsHandler, ListReadingsStatsHandler, GetReadingHandler, CreateReadingHandler, CreateReadingBulkHandler, UpdateReadingHandler, DeleteReadingHandler } from "../types";
...
const FIELDS: Spec[] = [{ name: "photo", type: "text" }, { name: "latitude", type: "real" }, { name: "longitude", type: "real" }, { name: "numeric_value", type: "real" }, { name: "timestamp", type: "date" }, { name: "encrypted_blob", type: "text" }, { name: "sync_status", type: "enum", values: ["pending","synced","failed","retrying"] }, { name: "sync_attempts", type: "integer" }, { name: "dedupe_id", type: "text" }, { name: "site_id", type: "integer" }, { name: "device_id", type: "integer" }, { name: "equipment_id", type: "integer" }, { name: "calibration_id", type: "integer" }];
...
const SORTABLE: string[] = ["id","photo","latitude","longitude","numeric_value","timestamp","encrypted_blob","sync_status","sync_attempts","dedupe_id","site_id","device_id","equipment_id","calibration_id"];
const SEARCHABLE: string[] = ["photo","timestamp","encrypted_blob","sync_status","dedupe_id"];
const FILTERABLE: string[] = ["site_id","device_id","equipment_id","calibration_id"];
const MATCHABLE: string[] = ["sync_status"];
const DATED: string[] = ["timestamp"];
const GROUPABLE: string[] = ["timestamp","sync_status","site_id","device_id","equipment_id","calibration_id"];
...
const HOPS: Record<string, { table: string; cols: string[] }> = {"site_id":{"table":"sites","cols":["last_sync","status","zone_id"]},"device_id":{"table":"devices","cols":["last_seen","status"]},"equipment_id":{"table":"equipments","cols":["type","installation_date","last_calibration","site_id","status","warranty_expiry"]},"calibration_id":{"table":"calibration_logs","cols":["equipment_id","calibrated_at","next_calibration","status","reading_id"]}};
...
export const listOutliers: ListOutliersHandler = async (req, env, params) => {
  const user = await requireSession(req, env);
  if (!user) return json({ error: "unauthenticated" }, 401);
  noteTenant(req, user.tenant);
  void params;
  const { results } = await env.DB.prepare("SELECT r.* FROM readings r JOIN sites s ON r.site_id = s.id WHERE r.numeric_value > s.mean_value + 2 * s.std_dev AND r.tenant = ? AND r.deleted_at IS NULL").bind(user.tenant).all<Row>();
  return json(results);
};

export const syncStatusHeatmap: SyncStatusHeatmapHandler = async (req, env, params) => {
  const user = await requireSession(req, env);
  if (!user) return json({ error: "unauthenticated" }, 401);
  noteTenant(req, user.tenant);
  void params;
  const { results } = await env.DB.prepare("SELECT latitude, longitude, COUNT(*) as count FROM readings WHERE sync_status = 'pending' AND tenant = ? AND deleted_at IS NULL GROUP BY latitude, longitude").bind(user.tenant).all<Row>();
  return json(results);
};

export const siteReadingTrend: SiteReadingTrendHandler = async (req, env, params) => {
  const user = await requireSession(req, env);
  if (!user) return json({ error: "unauthenticated" }, 401);
  noteTenant(req, user.tenant);
  void params;
  const { results } = await env.DB.prepare("SELECT DATE(timestamp) as day, AVG(numeric_value) as avg_value FROM readings WHERE site_id = ? AND tenant = ? AND deleted_at IS NULL GROUP BY DATE(timestamp) ORDER BY day").bind(params.id, user.tenant).all<Row>();
  return json(results);
};

export const pendingSyncReadings: PendingSyncReadingsHandler = async (req, env, params) => {
  const user = await requireSession(req, env);
  if (!user) return json({ error: "unauthenticated" }, 401);
  noteTenant(req, user.tenant);
  void params;
  const { results } = await env.DB.prepare("SELECT * FROM readings WHERE sync_status = 'pending' AND sync_attempts < 5 AND tenant = ? AND deleted_at IS NULL ORDER BY timestamp ASC").bind(user.tenant).all<Row>();
  return json(results);
};

export const failedSyncThresholdReadings: FailedSyncThresholdReadingsHandler = async (req, env, params) => {
  const user = await requireSession(req, env);
  if (!user) return json({ error: "unauthenticated" }, 401);
  noteTenant(req, user.tenant);
  void params;
  const { results } = await env.DB.prepare("SELECT * FROM readings WHERE sync_status = 'failed' AND sync_attempts >= 5 AND tenant = ? AND deleted_at IS NULL").bind(user.tenant).all<Row>();
  return json(results);
};

export const dedupeCheck: DedupeCheckHandler = async (req, env, params) => {
  const user = await requireSession(req, env);
  if (!user) return json({ error: "unauthenticated" }, 401);
  noteTenant(req, user.tenant);
  void params;
  const { results } = await env.DB.prepare("SELECT dedupe_id, COUNT(*) as count FROM readings WHERE tenant = ? AND deleted_at IS NULL GROUP BY dedupe_id HAVING COUNT(*) > 1").bind(user.tenant).all<Row>();
  return json(results);
};
...
export const createReading: CreateReadingHandler = async (req, env) => {
  ...
  try {
    const res = await env.DB.prepare("INSERT INTO readings (photo, latitude, longitude, numeric_value, timestamp, encrypted_blob, sync_status, sync_attempts, dedupe_id, site_id, device_id, equipment_id, calibration_id, tenant) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(body.photo ?? null, body.latitude ?? null, body.longitude ?? null, body.numeric_value ?? null, body.timestamp ?? null, body.encrypted_blob ?? null, body.sync_status ?? null, body.sync_attempts ?? null, body.dedupe_id ?? null, body.site_id ?? null, body.device_id ?? null, body.equipment_id ?? null, body.calibration_id ?? null, user.tenant).run();
...
export const createReadingBulk: CreateReadingBulkHandler = async (req, env) => {
  ...
  try {
    const stmt = env.DB.prepare("INSERT INTO readings (photo, latitude, longitude, numeric_value, timestamp, encrypted_blob, sync_status, sync_attempts, dedupe_id, site_id, device_id, equipment_id, calibration_id, tenant) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING id");
    const out = await env.DB.batch<{ id: number }>(
      rows.map((body) => stmt.bind(body.photo ?? null, body.latitude ?? null, body.longitude ?? null, body.numeric_value ?? null, body.timestamp ?? null, body.encrypted_blob ?? null, body.sync_status ?? null, body.sync_attempts ?? null, body.dedupe_id ?? null, body.site_id ?? null, body.device_id ?? null, body.equipment_id ?? null, body.calibration_id ?? null, user.tenant)),
    );
...
export const updateReading: UpdateReadingHandler = async (req, env, params) => {
  ...
  let sql = "UPDATE readings SET photo = COALESCE(?, photo), latitude = COALESCE(?, latitude), longitude = COALESCE(?, longitude), numeric_value = COALESCE(?, numeric_value), timestamp = COALESCE(?, timestamp), encrypted_blob = COALESCE(?, encrypted_blob), sync_status = COALESCE(?, sync_status), sync_attempts = COALESCE(?, sync_attempts), dedupe_id = COALESCE(?, dedupe_id), site_id = COALESCE(?, site_id), device_id = COALESCE(?, device_id), equipment_id = COALESCE(?, equipment_id), calibration_id = COALESCE(?, calibration_id), row_version = row_version + 1 WHERE id = ? AND tenant = ? AND deleted_at IS NULL";
  const args: unknown[] = [body.photo ?? null, body.latitude ?? null, body.longitude ?? null, body.numeric_value ?? null, body.timestamp ?? null, body.encrypted_blob ?? null, body.sync_status ?? null, body.sync_attempts ?? null, body.dedupe_id ?? null, body.site_id ?? null, body.device_id ?? null, body.equipment_id ?? null, body.calibration_id ?? null, params.id, user.tenant];
