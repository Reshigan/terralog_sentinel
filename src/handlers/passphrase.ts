...
import type { Passphrase, Bucket, ListPassphrasesHandler, ListPassphrasesStatsHandler, GetPassphraseHandler, CreatePassphraseHandler, CreatePassphraseBulkHandler, UpdatePassphraseHandler, DeletePassphraseHandler } from "../types";
...
const FIELDS: Spec[] = [{ name: "hash", type: "text" }, { name: "salt", type: "text" }, { name: "is_set", type: "bool" }, { name: "set_at", type: "date" }, { name: "device_id", type: "integer" }, { name: "failed_attempts", type: "integer" }];
function validate(body: Partial<Passphrase>, partial = false): string | null {
...
const SORTABLE: string[] = ["id","hash","salt","is_set","set_at","device_id","failed_attempts"];
const SEARCHABLE: string[] = ["hash","salt","set_at"];
const FILTERABLE: string[] = ["device_id"];
const MATCHABLE: string[] = [];
const DATED: string[] = ["set_at"];
const GROUPABLE: string[] = ["is_set","set_at","device_id"];
const MEASURABLE: string[] = ["failed_attempts"];
...
export const listPassphrases: ListPassphrasesHandler = async (req, env) => {
  ...
  const { where, args } = sc;
  const counted = await env.DB.prepare("SELECT COUNT(*) AS n, COALESCE(SUM(failed_attempts), 0) AS s_failed_attempts FROM passphrases WHERE " + where).bind(...args).first<Record<string, number>>();
  let pageWhere = where;
  const pageArgs = [...args];
  ...
  const { results } = await env.DB.prepare(
    "SELECT * FROM passphrases WHERE " + pageWhere + " ORDER BY " + sort + " " + dir + " LIMIT ? OFFSET ?",
  ).bind(...pageArgs, per, cursor > 0 && sort === "id" ? 0 : (page - 1) * per).all<Passphrase>();
  ...
  if (counted) out.headers.set("x-totals", JSON.stringify({ failed_attempts: counted.s_failed_attempts ?? 0 }));
  return out;
};

export const listPassphrasesStats: ListPassphrasesStatsHandler = async (req, env) => {
  const user = await requireSession(req, env);
  if (!user) return json({ error: "unauthenticated" }, 401);
  noteTenant(req, user.tenant);
  const url = new URL(req.url);
  ...
  const from = hop
    ? "passphrases JOIN (SELECT id AS \"hop.id\", " + hopCol + " AS \"hop.key\" FROM " + hop.table + " WHERE tenant = ? AND deleted_at IS NULL) ON \"hop.id\" = " + by.slice(0, dot)
    : "passphrases";
  const { results } = await env.DB.prepare(
...
export const getPassphrase: GetPassphraseHandler = async (req, env, params) => {
  const user = await requireSession(req, env);
  if (!user) return json({ error: "unauthenticated" }, 401);
  noteTenant(req, user.tenant);
  const row = await env.DB.prepare("SELECT * FROM passphrases WHERE id = ? AND tenant = ? AND deleted_at IS NULL").bind(params.id, user.tenant).first<Passphrase>();
...
export const createPassphrase: CreatePassphraseHandler = async (req, env) => {
  const user = await requireSession(req, env);
  if (!user) return json({ error: "unauthenticated" }, 401);
  noteTenant(req, user.tenant);
  const denied = requireRole(user, WRITE_ROLES);
  if (denied) return denied;
  const body = await readJson<Partial<Passphrase>>(req);
  ...
  if (key) {
    const seen = await env.DB.prepare("SELECT row_id FROM idempotency WHERE tenant = ? AND key = ? AND endpoint = ?").bind(user.tenant, key, "createPassphrase").first<{ row_id: number }>();
    if (seen) {
      const prior = await env.DB.prepare("SELECT * FROM passphrases WHERE id = ? AND tenant = ? AND deleted_at IS NULL").bind(seen.row_id, user.tenant).first<Passphrase>();
      if (prior) {
  ...
  try {
    const res = await env.DB.prepare("INSERT INTO passphrases (hash, salt, is_set, set_at, device_id, failed_attempts, tenant) VALUES (?, ?, ?, ?, ?, ?, ?)").bind(body.hash ?? null, body.salt ?? null, body.is_set ?? null, body.set_at ?? null, body.device_id ?? null, body.failed_attempts ?? null, user.tenant).run();
    const row = await env.DB.prepare("SELECT * FROM passphrases WHERE id = ?").bind(res.meta.last_row_id).first<Passphrase>();
    if (key)
      await env.DB.prepare("INSERT OR IGNORE INTO idempotency (tenant, key, endpoint, row_id, at) VALUES (?, ?, ?, ?, ?)")
        .bind(user.tenant, key, "createPassphrase", Number(res.meta.last_row_id), new Date().toISOString()).run();
    await audit(env, user, "create", "passphrase", Number(res.meta.last_row_id), row);
...
export const createPassphraseBulk: CreatePassphraseBulkHandler = async (req, env) => {
  const user = await requireSession(req, env);
  if (!user) return json({ error: "unauthenticated" }, 401);
  noteTenant(req, user.tenant);
  const denied = requireRole(user, WRITE_ROLES);
  if (denied) return denied;
  const payload = await readJson<{ rows?: Partial<Passphrase>[] }>(req);
  ...
  for (let i = 0; i < rows.length; i++) {
    const bad = validate(rows[i] as Partial<Passphrase>);
    if (bad) return json({ error: "row " + i + ": " + bad }, 400);
  }
  try {
    const stmt = env.DB.prepare("INSERT INTO passphrases (hash, salt, is_set, set_at, device_id, failed_attempts, tenant) VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING id");
    const out = await env.DB.batch<{ id: number }>(
      rows.map((body) => stmt.bind(body.hash ?? null, body.salt ?? null, body.is_set ?? null, body.set_at ?? null, body.device_id ?? null, body.failed_attempts ?? null, user.tenant)),
    );
    const ids = out.flatMap((r) => (r.results ?? []).map((x) => x.id));
    for (const id of ids) await audit(env, user, "create", "passphrase", id, null);
...
export const updatePassphrase: UpdatePassphraseHandler = async (req, env, params) => {
  const user = await requireSession(req, env);
  if (!user) return json({ error: "unauthenticated" }, 401);
  noteTenant(req, user.tenant);
  const denied = requireRole(user, WRITE_ROLES);
  ...
  let sql = "UPDATE passphrases SET hash = COALESCE(?, hash), salt = COALESCE(?, salt), is_set = COALESCE(?, is_set), set_at = COALESCE(?, set_at), device_id = COALESCE(?, device_id), failed_attempts = COALESCE(?, failed_attempts), row_version = row_version + 1 WHERE id = ? AND tenant = ? AND deleted_at IS NULL";
  const args: unknown[] = [body.hash ?? null, body.salt ?? null, body.is_set ?? null, body.set_at ?? null, body.device_id ?? null, body.failed_attempts ?? null, params.id, user.tenant];
  if (Number.isInteger(want)) { sql += " AND row_version = ?"; args.push(want); }
  try {
    const res = await env.DB.prepare(sql).bind(...args).run();
    if (!res.meta.changes) {
      const cur = await env.DB.prepare("SELECT row_version FROM passphrases WHERE id = ? AND tenant = ? AND deleted_at IS NULL").bind(params.id, user.tenant).first<{ row_version: number }>();
    ...
    const row = await env.DB.prepare("SELECT * FROM passphrases WHERE id = ? AND tenant = ? AND deleted_at IS NULL").bind(params.id, user.tenant).first<Passphrase>();
    if (row) await audit(env, user, "update", "passphrase", Number(params.id), body);
...
export const deletePassphrase: DeletePassphraseHandler = async (req, env, params) => {
  const user = await requireSession(req, env);
  if (!user) return json({ error: "unauthenticated" }, 401);
  noteTenant(req, user.tenant);
  const denied = requireRole(user, WRITE_ROLES);
  if (denied) return denied;
  const res = await env.DB.prepare("UPDATE passphrases SET deleted_at = ? WHERE id = ? AND tenant = ? AND deleted_at IS NULL").bind(new Date().toISOString(), params.id, user.tenant).run();
  if (res.meta.changes) await audit(env, user, "delete", "passphrase", Number(params.id), null);
  return res.meta.changes ? json({ ok: true }) : json({ error: "not found" }, 404);
};

export const deletePassphraseRestore: DeletePassphraseHandler = async (req, env, params) => {
  const user = await requireSession(req, env);
  if (!user) return json({ error: "unauthenticated" }, 401);
  noteTenant(req, user.tenant);
  const denied = requireRole(user, ADMIN_ROLES);
  if (denied) return denied;
  const res = await env.DB.prepare("UPDATE passphrases SET deleted_at = NULL WHERE id = ? AND tenant = ? AND deleted_at IS NOT NULL").bind(params.id, user.tenant).run();
  if (res.meta.changes) await audit(env, user, "restore", "passphrase", Number(params.id), null);
  return res.meta.changes ? json({ ok: true }) : json({ error: "not found" }, 404);
};
